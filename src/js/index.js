...
inputs.forEach((input) => {
    input.addEventListener("input", (e) => {
        const inputField = e.target.nextSibling.parentElement;
        if (!input.value) {
            inputField.classList.add("error-blank")
            input.classList.remove("activated")
            input.style.borderColor = '#ff5252'
        } else {
            inputField.classList.remove("error-blank")
...
inputsNumbers.forEach((input) => {
    input.addEventListener("change", (e) => {
        ...
        if (isNaN(input.value)) {
            inputField.classList.add("error-wrong")
            input.classList.remove("activated")
            input.style.borderColor = '#ff5252'
        } else {
            inputField.classList.remove("error-wrong")
            input.classList.add("activated")
        }
    })
})
